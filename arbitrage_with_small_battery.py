...
BATTERY_CAPACITY_WH = 100_000  # 100kWh
BATTERY_ENERGY_PER_INTERVAL = 5_000  # 5kWh
BATTERY_POWER_W = 60_000 # 60kW, derived from 5kWh per 5-min interval
...
def pre_calculate_optimal_thresholds(profitability_data):
    ...
    # Buy when energy is in the cheapest 10% of the week
    buy_price_threshold = np.percentile(all_energy_prices, 10)
    # Sell when energy is in the most expensive 10% of the week
    sell_price_threshold = np.percentile(all_energy_prices, 90)
    ...
    print(f"Buy energy when price < ${buy_price_threshold / 1000:.4f}/kWh (10th percentile +/- {THRESHOLD_BUFFER_PERCENT*100}%)")
    print(f"Sell energy when price > ${sell_price_threshold / 1000:.4f}/kWh (90th percentile +/- {THRESHOLD_BUFFER_PERCENT*100}%)")
    print("-----------------------------------------\n")
